# Reference Dockerfile for the Crossing Challenge.
...
#
# Run tests/smoke.py locally after building to verify the predict contract:
...
# unpickle model.pkl at build time — that would run untrusted candidate code
# on the grader host before any sandbox applies. The first docker-run
# invocation is the smoke test; it runs inside the sandboxed grader container.
